...
| **Round half DOWN**, `ceil(x - 0.5)` | Over −50..0 every odd percentage lands exactly on a half-dB, so the tie rule decides ~49.5 % of inputs. Volume must land quieter than asked. Holds for either sign, unlike round-half-away-from-zero | Python's `round()` — banker's rounding, wrong at every tie in a *different* way |
