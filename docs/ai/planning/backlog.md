...
| `M5-01` | **`monolith-htp1` skill authored** | `.claude/skills/monolith-htp1/SKILL.md`, 164 lines, shaped like `somfy-sdn`: transport, the trap that costs a physical trip, volume rules, absence and asymmetry, a 19-row "common mistake → consequence" table, and the Home Assistant specifics this device forces. Carved out of the `.claude/skills/*` ignore rule, so it ships with the public repo. Privacy-checked: no addresses, unit names or input labels. |
| `M5-06` | Durable findings stored in ai-devkit memory | `project:ha-monolith-htp1`: the power model (sleep vs shutdown, and the two traps that produced the original error) and the lip-sync dual write. `global`: reaching Home Assistant from a DNS-blocked sandbox with a pinned resolver rather than by disabling TLS verification. |
